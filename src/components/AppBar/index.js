...
const renderAppBarElements = ({ config, isLoggedOn, ...otherProps }) => {
  const renderIfFlag = renderIf(config.includes('flag'))
  const renderIfRole = renderIf(config.includes('role'))
  const renderIfYard = renderIf(config.includes('yard'))
  const renderIfPhone = renderIf(config.includes('phone'))
